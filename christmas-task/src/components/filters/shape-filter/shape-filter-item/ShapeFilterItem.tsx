...
export default function ShapeFilterItem({ shapeOption, shapes }: Props) {
    ...
    return (
        <label className="shape-filter__shape-group" htmlFor={`shape_${shapeOption}`}>
            <input
                className="shape-filter__checkbox"
                checked={shapes.value[shapeOption]}
                id={`shape_${shapeOption}`}
                value={shapeOption}
                type="checkbox"
                name=""
                onChange={handleChange}
            />
            <span className={`shape-filter__shape-item shape-filter__shape-item_${shapeOption}`} />
