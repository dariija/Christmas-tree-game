...
export default function ColourFilterItem({ colourOption, colours }: Props) {
    ...
    return (
        <label className="colour-filter__colour-group" htmlFor={`filter_${colourOption}`}>
            <input
                className="colour-filter__checkbox"
                checked={colours.value[colourOption]}
                id={`filter_${colourOption}`}
                value={colourOption}
                type="checkbox"
                name=""
                onChange={handleChange}
            />
            <span className={`colour-filter__colour-item colour-filter__colour-item_${colourOption}`} />
        </label>
    );
}
